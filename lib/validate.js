...
function validate( opts, options ) {
	...
	if ( options.hasOwnProperty( 'b' ) ) {
		...
		if ( !isNumber( opts.b ) || opts.b <= ( opts.a || 0 ) ) {
			return new TypeError( 'mgf()::invalid option. `b` parameter must be a number primitive greater than `a`. Option: `' + opts.b + '`.' );
		}
	}
	if ( options.hasOwnProperty( 'c' ) ) {
		opts.c = options.c;
		if ( !isNumber( opts.c ) || opts.c < ( opts.a || 0 ) || opts.c > ( opts.b || 1 ) ) {
			return new TypeError( 'mgf()::invalid option. `c` parameter must be a number primitive on the interval `[a,b]`. Option: `' + opts.c + '`.' );
		}
	}
